...
class TSCH_EXEC:
    def __init__(self, username='', password='', domain='', hashes=None, aesKey=None, doKerberos=False, kdcHost=None,
                 command=None, sessionId=None, silentCommand=False, messg=''):
    ...
    def doStuff(self, rpctransport):
        def output_callback(data):
            try:
                log = open('./logs/attack_log.log','a')
                output = open('./logs/debug_log.log','a')
                data = data.decode(CODEC).replace(',','',2)
                data = data.replace('AM','',1)
                data = data.replace('PM','',1)
                log.write('\n' + self.__messg)
                output.write('\n\n' + self.__messg + '\n')
...
def main(auth,cmand,messg):
    parser = argparse.ArgumentParser()
    CODEC = 'utf-8'
    parser.add_argument('-messg', action='store', default=messg, help='messg')
    parser.add_argument('-target', action='store', default=auth, help='[[domain/]username[:password]@]<targetName or address>')
    parser.add_argument('-command', action='store', nargs='*', default=cmand, help='command to execute at the target ')
    ...
    atsvc_exec = TSCH_EXEC(username, password, domain, options.hashes, options.aesKey, options.k, options.dc_ip,
                           options.command, options.session_id, options.silentcommand, options.messg)
