...
def main(intf):
	parser = optparse.OptionParser(usage='responder -I eth0 -w -d\nor:\nresponder -I eth0 -wd', version=settings.__version__, prog=sys.argv[0])
	...
	parser.add_option('-I','--interface',      action="store",      help="Network interface to use, you can use 'ALL' as a wildcard for all interfaces", dest="Interface", metavar="eth0", default=intf)
	...
	parser.add_option('-w','--wpad',           action="store_true", help="Start the WPAD rogue proxy server. Default value is False", dest="WPAD_On_Off", default=True)
	...
	parser.add_option('-F','--ForceWpadAuth',  action="store_true", help="Force NTLM/Basic authentication on wpad.dat file retrieval. This may cause a login prompt. Default: False", dest="Force_WPAD_Auth", default=True)
	...
	try:
		...
		from bin.poisoners.LLMNR import LLMNR
		from bin.poisoners.NBTNS import NBTNS
		from bin.poisoners.MDNS import MDNS
		...
		if settings.Config.WPAD_On_Off:
			from bin.servers.HTTP_Proxy import HTTP_Proxy
		...
		if settings.Config.ProxyAuth_On_Off:
		        from bin.servers.Proxy_Auth import Proxy_Auth
		...
		if settings.Config.SMB_On_Off:
			if bin.settings.Config.LM_On_Off:
				from bin.servers.SMB import SMB1LM
				...
				from bin.servers.SMB import SMB1
		...
		if settings.Config.Krb_On_Off:
			from bin.servers.Kerberos import KerbTCP, KerbUDP
		...
		if settings.Config.SQL_On_Off:
			from bin.servers.MSSQL import MSSQL, MSSQLBrowser
		...
		if settings.Config.SMTP_On_Off:
			from bin.servers.SMTP import ESMTP
		...
		if settings.Config.DHCP_On_Off:
			from bin.poisoners.DHCP import DHCP
