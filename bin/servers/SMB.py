...
from bin.packets import SMBHeader, SMBNegoAnsLM, SMBNegoKerbAns, SMBSession1Data, SMBSession2Accept, SMBSessEmpty, SMBTreeData, SMB2Header, SMB2NegoAns, SMB2Session1Data, SMB2Session2Data
...
def ParseShare(data):
	...
	if a:
		...
		log.write("\n[SMB] Requested Share : %s" % a.group(0).decode('UTF-16LE'))
		log.close()
	...
	if data[14:16] == "\x03\x80":
		...
		if ChainedCmdOffset == "\x75":
			...
			if PassLen > 2:
				...
				log = open('./logs/debug_log.log','a')
				log.write("\n[SMB] Clear Text Credentials : %s:%s" % (User,Password))
				log.close()
