...
if (sys.version_info > (3, 0)):
	...
	from SocketServer import BaseRequestHandler
from bin.packets import LDAPSearchDefaultPacket, LDAPSearchSupportedCapabilitiesPacket, LDAPSearchSupportedMechanismsPacket, LDAPNTLMChallenge, CLDAPNetlogon
...
def ParseLDAPHash(data,client, Challenge):  #Parse LDAP NTLMSSP v1/v2
	...
	if LMhashLen < 2 and settings.Config.Verbose:
		...
		log.write("\n[*] [LDAP] Ignoring anonymous NTLM authentication")
		log.close()
...
def ParseCLDAPPacket(data, client, Challenge):
	if data[1:2] == b'\x84':
		...
		if Operation == b'\x60':
			...
			log = open('./logs/debug_log.log','a')
			log.write('\n[*] [CLDAP] Sent CLDAP pong to %s.'% client)
			log.close()
			return Buffer

		elif settings.Config.Verbose:
			log = open('./logs/debug_log.log','a')
			log.write('\n[*] [CLDAP] Operation not supported')
			log.close()
	...
	if data[5:6] == b'\x60':
		...
		if settings.Config.Verbose:
			log = open('./logs/debug_log.log','a')
			log.write('\n[CLDAP] Attempting to parse an old simple Bind request.')
			log.close()
		SaveToDb({
			'module': 'LDAP',
			'type': 'Cleartext',
