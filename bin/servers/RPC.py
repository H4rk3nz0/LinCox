...
from bin.packets import RPCMapBindAckAcceptedAns, RPCMapBindMapperAns, RPCHeader, NTLMChallenge, RPCNTLMNego
		...
		try:
			...
			if data[0:3] == b"\x05\x00\x00":#Mapper Response.
				...
				if NetworkSendBufferPython2or3(DSRUAPI) in data:
					...
					log.write("\n[*] [DCE-RPC Mapper] Redirected %-15sto DSRUAPI auth server." % (self.client_address[0]))
					log.close()
					self.request.close()
				...
				if NetworkSendBufferPython2or3(LSARPC) in data:
					...
					data = self.request.recv(1024)
					log = open('./logs/debug_log.log','a')
					log.write("\n[*] [DCE-RPC Mapper] Redirected %-15sto LSARPC auth server." % (self.client_address[0]))
					log.close()
					self.request.close()
				...
				if NetworkSendBufferPython2or3(WINSPOOL) in data:
					...
					data = self.request.recv(1024)
					log = open('./logs/debug_log.log','a')
					log.write("\n[*] [DCE-RPC Mapper] Redirected %-15sto WINSPOOL auth server." % (self.client_address[0]))
					log.close()
					self.request.close()
				...
				if NetworkSendBufferPython2or3(NETLOGON) in data:
					...
					#data = self.request.recv(1024)
					log = open('./logs/debug_log.log','a')
					log.write("\n[*] [DCE-RPC Mapper] Redirected %-15sto NETLOGON auth server." % (self.client_address[0]))
					log.close()
